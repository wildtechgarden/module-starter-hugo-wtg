...
systemd-run --working-directory "$(pwd)/tests/config" -E PATH="${PATH}" -E HUGO_RESOURCEDIR="$(pwd)"/resources -E HUGO_MODULE_REPLACEMENTS="${HUGO_MODULE_REPLACEMENTS}" --unit=hugo-serve --user hugo serve --source $(pwd) --environment "production" --config "$(pwd)"/tests/config/hugo.toml --port 1313 --bind 127.0.0.1
